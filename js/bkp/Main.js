...
function init() {
    ...
    // testdata[4].objects = [ { type: "wall", param: "r" } ];
    testdata[5].objects = [ { type: "wall", param: "l" } ];
    // testdata[5].objects = [ { type: "crate" } ];
        ...
        for (var i = 0; i < map.tiles.length; i++) {
            if (mouse.isInRect(map.tiles[i].rect)) {
                ...
                for (var j = 0; j < p.length; j++) {
                    var point = new Point(p[j].rect.x, p[j].rect.y);
                    point.fill = "rgba(255,0,255,." + (j * 10 + 30) + ")";
                    path.push(point);
                };
    ...
    function isPassableWay(tileA, tileB, direction) {
        ...
        for (var i = 0; i < tileB.objects.length; i++) {
            if (tileB.objects[i] instanceof Wall) {
                sum = direction + tileB.objects[i].side;
                // diagonal direction
                if (direction.length == 2) {
                    // test tileB
                    if (direction.indexOf(tileB.objects[i].side) == -1) {
                        return false;
                    }
        ...
        if (direction.length == 2) {
            for (var c = 0; c < 2; c++) {
                nextTilesPassable = checkNextTiles(tileA.nextTiles[direction.charAt(c)], direction, direction.charAt(c));
            }
            if (!nextTilesPassable) return false;
        }

        function checkNextTiles(next, direction, subDirection) {
            ...
            for (var i = 0; i < next.objects.length; i++) {
                if (next.objects[i] instanceof Wall) {
                    sum = subDirection + direction + next.objects[i].side;
                    for (var j = 0; j < imposibleDirections.length; j++) {
                        if (sum == imposibleDirections[j]) return false;
                    }
                } else if (!next.objects[i].isPassable) {
                    return false;
                }
            }
